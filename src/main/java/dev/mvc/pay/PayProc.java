...
public class PayProc implements PayProcInter {
    ...
    public int create(PayVO payVO) {
      int cnt = 0;
      cnt = this.payDAO.create(payVO);
      return cnt;
    }

    @Override
    public List<PayVO> list_all() {
      List<PayVO> list = this.payDAO.list_all();
      return list;
    }
    ...
    public int delete(int pay_payno) {
      int cnt = 0;
      cnt = this.payDAO.delete(pay_payno);
      return cnt;
    }
    ...
    public int delete_catebag(int catebag_catebagno) {
      int cnt = 0;
      cnt = this.payDAO.delete_catebag(catebag_catebagno);
      return cnt;
    }
    
    @Override
    public List<Game_Pay_VO> game_pay_consumer_no_list(int consumer_no) {
      List<Game_Pay_VO> list = this.payDAO.game_pay_consumer_no_list(consumer_no);
      return list;
    }
    ...
    public PayVO read(int pay_payno) {
      PayVO payVO = null;
      payVO = this.payDAO.read(pay_payno);
      return payVO;
    }
    ...
    public List<Game_Pay_VO> pay_by_consumer_no_paging(HashMap<String, Object> map) {
      ...
      */
      int beginOfPage = ((Integer)map.get("nowPage") - 1) * Pay.RECORD_PER_PAGE;
      ...
      */
      ...
      List<Game_Pay_VO> list = this.payDAO.pay_by_consumer_no_paging(map);
      
   // --------------------------------------------------------------
      // 내용중에 100자가 넘어가면 앞부분 100자만 추출
      // --------------------------------------------------------------
      String game_writing = "";
      for (Game_Pay_VO game_Pay_VO: list) {
        game_writing = game_Pay_VO.getGame_writing();
        if (game_writing.length() >= 100) {
          game_writing = Tool.textLength(game_writing, 100);
          game_Pay_VO.setGame_writing(game_writing);
        }
      }
      // --------------------------------------------------------------    
    ...
    public String pagingBox(String listFile, int search_count, int nowPage){ 
      int totalPage = (int)(Math.ceil((double)search_count/Pay.RECORD_PER_PAGE)); // 전체 페이지  
      int totalGrp = (int)(Math.ceil((double)totalPage/Pay.PAGE_PER_BLOCK));// 전체 그룹 
      int nowGrp = (int)(Math.ceil((double)nowPage/Pay.PAGE_PER_BLOCK));    // 현재 그룹 
      int startPage = ((nowGrp - 1) * Pay.PAGE_PER_BLOCK) + 1; // 특정 그룹의 페이지 목록 시작  
      int endPage = (nowGrp * Pay.PAGE_PER_BLOCK);             // 특정 그룹의 페이지 목록 종료   
