...
<td style='width: 400px;'>      
     <c:choose>
     <c:when test="${sessionScope.consumer_id == null && sessionScope.admin_id == null}">
     ...
     <c:when test="${sessionScope.consumer_id != null && sessionScope.admin_id == null}">
        ${sessionScope.consumer_id } (${sessionScope.consumer_nickname })님 아뇽하세요
        <button type="button" onclick="location.href='/team5/consumer/logout.do'"  class="btn btn-default pull-right">로그아웃</button>
     </c:when>
     <c:when test="${sessionScope.consumer_id == null && sessionScope.admin_id != null}">
        ${sessionScope.admin_id } (관리자)님 아뇽하세요<br>
        <button type="button" onclick="location.href='/team5/consumer/create.do'"  class="btn btn-default pull-left">회원가입</button>
        <button type="button" onclick="location.href='/team5/consumer/list.do'"  class="btn btn-default pull-left">회원관리</button>
        <button type="button" onclick="location.href='/team5/consumer/logout.do'"  class="btn btn-default pull-right">로그아웃</button>
     </c:when>
    </c:choose>
...
<div  style='background-color:#1b2838 '>
  ...
  <span class='abc' onclick="document.getElementById('id01').style.display='none'"  title="Close Modal" style='font-size: 30px;'>×</span>
    ...
    <div style='width: 70%; margin: 0px auto; padding:10px 0px;'>
      <input id='id' type="text" placeholder="아이디" name="id" style='width: 100%; height: 30px;'  >
    </div>
    <div style='width: 70%; margin: 0px auto; padding:10px 0px;'>
       <input id='passwd' type="password" placeholder="패스워드" name="passwd" style='width: 100%; height: 30px;' >
    </div>  
 </div>
 </div>
