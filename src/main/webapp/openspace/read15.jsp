...
<script type="text/javascript">
  ...
  function reply_create() {
    ...
    var frm = $('#frm_reply');  // Youtube 삭제 폼   
    var word = $('#word',frm).val();
    var search = $('#search',frm).val(); 
    var nowPage = $('#nowPage',frm).val();
    ...
    var params ='openspace_no='+openspace_no+'&search='+search+'&word='+word+'&nowPage='+nowPage+'&openspace_reply_content='+openspace_reply_content+'&consumer_no='+consumer_no;
    var msg = '';
...
function replyupdate(no, content, pk) {
  ...
  tag += "<textarea name='openspace_reply_update' id='openspace_reply_update"+pk+"'  style='background-color:#111923; width: 100%; height: 200px;' rows='10' cols='30'>"+content+"</textarea>";
...
댓글 쓰기
          ...
          <div style='margin: 0px auto; width: 90%; height: 310px; background-color: #131E2C; margin-top:20px;'>
            <c:choose>
              <c:when test="${sessionScope.consumer_id == null}">
                ...
                <button class='pull-right' style='margin-right: 60px; padding: 5px 15px; font-size: 20px;'  type="button" onclick="javascript:noconsumer()">등록</button>
                  ...
                  <c:otherwise>
                    ...
                    <button type="button" id='btn_send' class='pull-right' style='margin-right: 60px; padding: 5px 15px; font-size: 20px;'>등록</button>                              
                  </c:otherwise>
            </c:choose>
          </div>
          </div>
        ...
        <c:if test="${info.count<=5 }">  
         ...
         <c:if test="${consumer_no==sessionScope.consumer_no ||  sessionScope.admin=='Y' }">
        ...
        <form name='frm' id='frm'>    
          ...
          <input type='hidden' name='search' id='search' value='${param.search }'>
          <input type='hidden' name='nowPage' id='nowPage' value='${param.nowPage }'>   
          <input type='hidden' name='openspace_reply_no' id='openspace_reply_no' value='${openspace_reply_no }'>   
