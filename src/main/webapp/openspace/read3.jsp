...
function panel_img(file) {
  var tag = "";
  tag ="<div style='width: 80%; margin: 0px auto;'>"; 
  ...
  <div class="row">
    ...
    <DIV style='width: 100%; margin: 0px auto; '>
      ...
      <div class="col-md-10"
        ...
        <a href ='../openspace_img/delete.do?openspace_no=${openspace_no }&openspace_img_no=${openspace_no_img }'> 파일삭제x</a>
        /
        ...
        <div>
          <table style="width: 100%; border-bottom: solid 1px #999999;">
             <colgroup>
              <col style='width: 10%;' />
              <col style='width: 90%;' />
            </colgroup>
<td style="font-weight: bold; text-align: center;" rowspan="${size+1 }">파일첨부</td>
<c:if test="${size ==0}">
<td>첨부된 파일이 없습니다.</td>
</c:if>
...
<div style="margin:50px; position: relative; height:0; padding-bottom: 56.25%;" class='AA'>
   ...
   <div id="myCarousel" class="carousel slide" data-ride="carousel" style="height: 350px; width: 90%; margin: 0px auto;">
    ...
    <c:if test="${info.count==1 }">
      <div class="item active">
        <IMG src='../openspace_img/storage/${img.openspace_img_fname }' alt="" style="width:100%; height: 350px;">
      </div>    
    </c:if>
    <c:if test="${info.count >1 }">
      <div class="item">
    ...
    <a class="right carousel-control" href="#myCarousel" data-slide="next">
      ...
      <div style='margin: 0px auto; width: 90%; border: solid;'>    
            <c:forEach var="img" items="${list }" varStatus="info">
              <c:set var="thumb" value="${img.openspace_img_thumb.toLowerCase() }" />

              <c:choose>
                <c:when test="${thumb.endsWith('jpg') || thumb.endsWith('png') || thumb.endsWith('gif')}">
                <a data-target="#myCarousel" data-slide-to="${info.count-1 }">
                  <IMG src='../openspace_img/storage/${img.openspace_img_thumb }' style='margin-top: 2px;'>
                  </A>
                </c:when>
              </c:choose>
            </c:forEach>
