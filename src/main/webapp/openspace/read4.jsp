...
<body>
  ...
  <div class="col-md-12" style=" margin-top: 16px; margin-bottom: 50px;">
    <c:if test="${youtube.length() > 0 }">
      ...
      <div style='margin: 0px auto; width: 80%; border: solid;'>    
            <c:forEach var="img" items="${list }" varStatus="info">
              <c:set var="thumb" value="${img.openspace_img_thumb.toLowerCase() }" />

              <c:choose>
                <c:when test="${thumb.endsWith('jpg') || thumb.endsWith('png') || thumb.endsWith('gif')}">
                  <A href="javascript:panel_img('${img.openspace_img_fname }')"><IMG src='../openspace_img/storage/${img.openspace_img_thumb }' style='margin-top: 2px;'></A>
                </c:when>
              </c:choose>
            </c:forEach>
