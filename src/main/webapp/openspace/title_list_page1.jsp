...
<div style="margin-left: 25px;">
  ...
  <div style="margin-bottom: 10px;">
    ...
    제목검색: ${word} 검색된 글 수${search_count } 건
    </c:if>
        <c:if test="${search == 'contents' }">
    내용검색: ${word} 검색된 글 수${search_count } 건
    </c:if>
        <c:if test="${search == 'name_contents' }">
    제목+내용검색: ${word} 검색된 글 수${search_count } 건
    </c:if>
        </c:when>
    ...
    <c:set var="redate" value="${openspaceVO.openspace_redate}" />
      ...
      <tr>
        ...
        <td style="text-align: left;">
         <a href="./read.do?openspace_no=${openspace_no}&search=${search }&word=${word}&nowPage=${nowPage}">${title }</a></td>
