   ...
   <c:forEach var="openspaceVO"  items="${list }" varStatus="info"> <!-- request 객체에 접근 -->
    ...
    <c:set var="consumer_nickname" value="${openspaceVO.consumer_nickname}" />
      ...
      <tr class ='${visible }'>
        ...
        <c:if test="${title.length()>34 }">
          <td class='${openspace_no } ' style="text-align: left;">
            <a class='QWER${info.count }' href="./read.do?openspace_no=${openspace_no}&search=${search }&word=${word}&nowPage=${nowPage}">${visible} ${title.substring(0,34) }...</a>
          </td>                
        </c:if>
        <c:if test="${title.length()<=34 }">
          <td class='${openspace_no } ' style="text-align: left;">
      ...
      <c:choose>
        ...
        <div style = "margin: 15px auto; text-align: center; margin-bottom: 20px;">
