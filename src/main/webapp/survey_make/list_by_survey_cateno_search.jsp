...
<jsp:include page="/menu/top3.jsp" flush='false' />
  ...
  <form name='frm' id='frm' method='get' action='./survey_list.do'>
    <input type='hidden' name='survey_cateno' value='${survey_cateVO.survey_cateno }'>
      ...
      <ASIDE style='float: right;'>
        ...
        <A href="javascript:location.reload();">새로고침</A>
         ...
         <c:choose>
        <c:when test="${param.survey_word != '' }"> 검색하는 경우
          <input type='text' name='survey_word' id='survey_word' value='${param.survey_word }' 
                     style='width: 35%;'>
        </c:when>
        <c:otherwise> 검색하지 않는 경우
          <input type='text' name='survey_word' id='survey_word' value='' style='width: 35%;'>
        </c:otherwise>
      </c:choose>
      <button type='submit'>검색</button>
      <c:if test="${param.survey_word.length() > 0 }">
        <button type='button' 
    ...
    <table class="table table-striped" style='width: 100%;'>
      ...
      <tbody>
        <c:forEach var="survey_makeVO" items="${list }">
          ...
          <tr> 
            <td style='text-align: center;'>${survey_makeVO.survey_wdate.substring(0, 10)}</td>
            <td style='text-align: center;'>${survey_makeVO.survey_cateno}</td>
            <td>
              <a href="./survey_read.do?survey_makeno=${survey_makeno}">${survey_makeVO.survey_question}</a> 
            </td> 
