...
<jsp:include page="/menu/top3.jsp" flush='false' />
  ...
  <DIV class='title_line'>
    ${param.survey_cate_survey_name }
  </DIV>
  
  <ASIDE style='float: left;'>
    ${param.survey_categrp_survey_name } > ${param.survey_cate_survey_name } > 글 수정
  </ASIDE>
...
<div style="background-color: #ff000; text-align: center;">
  <fieldset class='fieldset_basic'>
    <UL>
      <c:choose>
        <c:when test="${param.survey_passwd_cnt == 1 }"> <!-- 패스워드 일치 -->
          <c:choose>
            <c:when test="${param.survey_cnt == 1}"> <!-- 글 수정 성공 -->
              <LI class='li_none'>
...
              </LI>
      ...
      <c:choose>
        <c:when test="${param.survey_cnt == 1 && param.survey_passwd_cnt == 1}">
          <LI class='li_none'>
            <button type='button' 
                        onclick="location.href='./survey_read.do?survey_cateno=${param.survey_cateno}&survey_makeno=${param.survey_makeno}'"
                        class="btn">변경 확인</button>
            <button type='button' 
