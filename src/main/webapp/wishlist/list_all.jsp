  ...
  <ASIDE style='float: right;'>
   ...
   <table class="table table-striped" style='width: 100%;'>
    <colgroup>
      ...
      <thead>
        <tr>
          <th style='text-align: center;'>위시리스트번호</th>
          <th style='text-align: center;'>상품번호</th>
          <th style='text-align: center;'>위시리스트 출력 순서</th>
      ...
      <thead>
      <tbody>
        <c:forEach var="wishlistVO" items="${list }">
          <c:set var="wishlist_wishlistno" value="${wishlistVO.wishlist_wishlistno }" />
          
          <tr> 
            <td style='text-align: center;'>${wishlistVO.wishlist_wishlistno}</td>
            <td style='text-align: center;'>${wishlistVO.list_contentsno}</td>
            <td style='text-align: center;'>${wishlistVO.wishlist_seqno}</td>
            <td style='text-align: center;'>${wishlistVO.consumer_no}</td>
            <td style='text-align: center;'>${wishlistVO.wishlist_date.substring(0, 10)}</td>
            ...
            <FORM name='frm' method='POST' action='/team5/catebag/create.do' class="form-horizontal">
                   
             <button type="submit" class="btn"><span class="glyphicon glyphicon-shopping-cart"></span></button>                      
            </FORM>  
            </td> 
          </tr>
        </c:forEach>       
